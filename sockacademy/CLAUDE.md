...
## 🤝 Consultative CTO Protocol — חובה מ-27/06/2026

**לפני כל Phase חדש, feature מרכזי, או החלטת ארכיטקטורה — חובה לשאול 1-3 שאלות אסטרטגיות.**

### כשמפעילים:
- לפני כל Layer חדש ב-Phase 4 UI (Layer 2, 3, 4)
- לפני כל agent חדש
- לפני שינוי ארכיטקטורה (schema, YAML, brand system)
- לפני כל החלטה שמשפיעה על UX / CRO / brand

### כשלא מפעילים:
- תיקוני bugs — מבצעים מיד
- שינויים קטנים שכבר אושרו (מבנה קיים, pattern שחוזר)
- המשך ישיר של task שגיא כבר אישר

### כללי השאלות:
1. שאלות על מה שעדיין **לא מוגדר** — לא על brand/Iron Laws שכבר קיימים
...
4. **DRY_RUN=true node agent.js עבר מקומית — חובה ריצה אמיתית שמגיעה לשורת ה-log הסופית (הצלחה או "DORMANT"), לא רק `node -c` syntax check (נקבע 04/07/2026, PROCESS-6, Fable 5 audit).** `node -c` בודק תחביר בלבד ולא היה תופס את באג ה-dotenv-path של A25 (ANTI_RECURRENCE #34) — רק ריצה אמיתית הייתה חושפת אותו. השתמש ב-`node scripts/setup/run-agent-safely.js <AgentId>` (PROCESS-2) שכובר default ל-DRY_RUN=true ומונע ריצה live בטעות.
