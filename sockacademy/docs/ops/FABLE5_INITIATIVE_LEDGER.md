...
- 2026-07-05 — Stage 17 (Claude Code / Desktop settings audit, Guy's ask from a screenshot): done — `sockacademy/docs/ops/FABLE5_STAGE17_CLAUDE_SETTINGS_AUDIT.md`. Section-by-section verdicts, config-verified where a local file backs the setting (MCP servers, skills, hooks) and flagged as general-knowledge-only where it's cloud-only (Account/Billing/Usage/Cowork/Claude in Chrome — no vision/browser access). Found the MCP/Skills verdict tables in `CLAUDE.md` had drifted from installed reality (several "ACTIVE" servers not actually installed; `magic` MCP installed with a live plaintext API key in `~/.claude.json` and unused). **Security incident this stage:** the subagent's own output doc printed a fragment of that real key — caught and redacted before staging (never reached git), logged as ANTI_RECURRENCE #42, dispatch prompts now explicitly instruct "report existence/location of a secret, never its value." Claude executed the safe fixes directly: CLAUDE.md tables corrected to match reality, unused `magic` MCP server removed from `~/.claude.json` (backup kept).
- 2026-07-06 — Stage 18 (target-state orchestration architecture — "one big orchestra", Guy's explicit ask, dispatched as its own stage per his choice over folding into Stage 19): done — `sockacademy/docs/ops/FABLE5_STAGE18_ORCHESTRATION_ARCHITECTURE.md`. Confirmed A0 has zero actuation calls today but the mechanism already exists proven in-repo (`shopify-webhook-handler.yml:95` → `createWorkflowDispatch`), so growing A0 into a conductor is a policy choice, not a capability gap. `queue.js` needs "Q-HARDEN" (ack-on-pop via LMOVE, `queue_log` lifecycle, retry+dead-letter) before anything transactional rides on it — packaged as A2.7's own first task, not a separate project. Target design: editorial/intel fleet stays on cron forever (Zero-Waste — no latency benefit to migrating); A2.7/A16.5 are the event-driven section per their existing skeleton entries, every event path shadowed by a daily reconciliation cron; A0 grows a 3-rung actuation ladder (Rung 1 allowlisted retry-once in propose→auto mode mirroring A2.7's HITL ramp; Rung 2 quarantine flapping agents; Rung 3 DAG/reordering explicitly DO NOT BUILD, named as the over-engineering line). **Unhedged devil's-advocate verdict: build none of it now** — cron + monitoring + Guy-as-exception-handler is the correct architecture at 0 orders, not a compromise; every phase names its own trigger (`PHASE_2_ACTIVATE_BY_GUY`/25 orders/$1K MRR → then 1mo clean → Phase 3 $5K MRR×2mo). The design itself was the deliverable Guy needed pre-positioned before order pressure hits.
- 2026-07-06 — Stage 19 (blind-spot discovery for a solo founder, categories A-E per the tracker's queued brief; 100% read-only): done — `sockacademy/docs/ops/FABLE5_STAGE19_BLIND_SPOTS.md`. Two genuinely-real findings, both externally verified this stage: (E2) **a same-name UK sock company exists** — "Sock Academy Ltd", Companies House 05743003, trading since 2006, holds sockacademy.com, novelty-sock brands in 18 countries; registered-mark status unverifiable from here → add as Q8 to the attorney packet *before* sending (1.3) + 30-min free USPTO/UKIPO/EUIPO/ILPO self-search; and (D4) **the brand domain cannot authenticate Shopify email today** — direct DNS queries confirmed no SPF record on sockacademy.store, Shopify DKIM CNAMEs (shop1/shop2._domainkey) NXDOMAIN, DMARC live at p=quarantine with reports going to GoDaddy's default mailbox (silent-failure by construction); Klaviyo's mail. subdomain is properly NS-delegated and fine — fix is ~15 min of Shopify Admin + GoDaddy DNS, best before first order. Honest de-escalations: storefront traffic spikes never touch Supabase/Upstash/Actions (store is 100% Shopify-hosted); core theme a11y is solid (Dawn spine intact, zero missing alt attrs, palette computes AA-pass) with the real gaps confined to sock-finder.liquid (JS-only render, no aria-live, focus loss on innerHTML swap, and Dawn's 1rem=10px base making quiz text compute at 6.5-9px) → bundle into the queued Sock Finder v2 pass (item #8). Also confirmed: zero data backup exists (only A5's image backup; no pg_dump anywhere) → manual monthly dump now, scheduled workflow at Phase 2 gate; bus-factor never addressed in any of the 42 protocols, with a verified day-60 cliff (GitHub auto-disables crons after 60 days repo inactivity, killing A0 with the fleet) → one sealed continuity note + Google Inactive Account Manager, ~2hrs; entity/insurance/IOSS all routed to the Phase 2 gate with named triggers, not manufactured urgency. Tooling: Shopify Dev MCP + read-only Supabase MCP proposed (config-only, freeze-safe); UptimeRobot confirmed named-but-never-acted-on. All recommendations freeze-compliant — nothing needs a new agent/table/workflow before Phase 2. **Controller-verified independently, same night:** D4 confirmed exactly via live `nslookup` (no root SPF TXT, `shop1/shop2._domainkey` both NXDOMAIN, DMARC `p=quarantine` with `rua=` at `onsecureserver.net`, `mail.` correctly NS-delegated to Klaviyo) — every claim matched. E2 confirmed via web search: Companies House 05743003 is active, incorporated 15/03/2006, registered office in Bedfordshire, website `sockacademy.com`, 12 employees, formerly named "United Oddsocks Limited" (2006-2017) — matching Fable's own citation of "United Oddsocks" as one of the company's three brands exactly. Trademark *registration* status remains unverified (correctly out of scope for a web search) — the USPTO/UKIPO/EUIPO/ILPO self-search and attorney-packet Q8 stand as the real next step.
- 2026-07-06 — Master Execution Protocol adopted as standing SOP (`feedback_fable5_execution_protocol.md` memory, refined from Guy's draft with 4 corrections: per-item-type quality bar not blanket Iron Law 2, hard rule to never touch 🧑/🧠-owned items without Guy present, mandatory memory-touch on every loop close, QA-gate rubric scoped to A3/A5 only). Phase 1 of the first execution pass under this protocol, done same session: **tracker item H** — built `scripts/ci/yaml-reality-audit.js`, a deterministic version of `/boot-sockacademy`'s manual cross-reference check (`c896907`); first real run found 3 pre-existing config gaps (A0's optional Gmail-personal-inbox read inert, A7's low-stock/price-change alerting gracefully disabled, A1's Make.com webhook correctly-optional false-positive) — reported for Guy's triage, not fixed. **Stage 19 item 9** — ran `@axe-core/cli` against 3 live pages (homepage/collection/product, `1e35b1b`); found a real, site-wide footer contrast+landmark defect (`sections/footer.liquid`'s `--muted`/`--ghost` on `--surface-void` fails AA in the browser, refining rather than contradicting §B2's separate `--surface-base` check) plus a heading-order slip and a third-party (Judge.me) ARIA issue — folded into the already-queued Sock Finder v2 batch, still gated on Design Freeze approval. **Stage 19 item 10** — installed and verified the official Shopify Dev MCP server (`77a0ebf`; first attempt hit a corrupted npx cache, fixed via `npm cache verify`); item 10b (read-only Supabase MCP) correctly left blocked — requires a Supabase account-level PAT only Guy can generate, deliberately not substituted with the higher-privilege `SUPABASE_SERVICE_KEY`.
- 2026-07-06 — Stage 20 (true unit economics + Day-1 manual fulfillment runbook, timed to land before Guy's pending 3.1/3.3 Keystone decisions; 100% read-only): done — `sockacademy/docs/ops/FABLE5_STAGE20_UNIT_ECONOMICS_AND_FULFILLMENT.md`. **Part 1, repo-verified:** every margin number the fleet has ever produced is raw `retail − supplier_price` (A1 `agent.js:596-597`, A15 `agent.js:97`; `products_table.sql` has no fee/shipping columns at all) — zero handling of payment fees, shipping, CJ costs, or refunds anywhere. True contribution model built (payment 2.9%+$0.30 [EXT — or ~5.5%+$0.49 if a third-party gateway is live, undeterminable from repo, Guy must check Settings→Payments], CJ per-order fee assumed $0 per CJ's published model [EXT], shipping ~$5/pair CJPacket-class [EST], 5% refund allowance [EST], supplier costs as documented examples — no Supabase access, not a live query): naive margins overstate by ~20-40 points; $18 floor → true CM ~$6.30/unit (34.9%, ~$4.70 on a WELCOME10 first order) vs. $24 → ~$11.80 (49.2%). Honest calibration: break-even is ~$11-12, nothing is underwater — the finding is overstated reporting + a thin floor, not a business-killing hole. **Unhedged recommendation: floor to $24** (top of memo 3.3's own band); explicitly neutral on 3.1 — corrected math changes the floor within either positioning, not which positioning is viable. Adjacent live-code find: A7's `MARKUP_MULTIPLIER=2.5×` auto-reprice can set retail below the $18 floor — inert today (thresholds null) but arms itself if `PRICE_WARN_PCT` is ever set; queued as a guard fix with the A15/A1 formula corrections (🤖, Guy-approved future session, freeze-compliant). **Part 2:** first-ever Day-1 runbook — verified no code notifies Guy of an order today (webhook→queue push has zero consumers; the real trigger is Shopify's staff email + mobile-app push, both external defaults); 7-item pre-flight (incl. gateway check, CJ-app-connection check, real-cj_pid check, D4 email fix), 7-step per-order procedure (pay-check → 1h cancellation window → cj_pid from `products` → CJ dashboard order → tracking → Shopify Fulfill-items writeback → Google Sheet log seeding A2.7's future `fulfillments` reconciliation), 3 customer templates in-register, printable 11pm checklist, and the scale ceiling: ~3/day sustained (or 25 cumulative — converges with the Phase 2 trigger on its own) = build A2.7 first, per Stage 18 O-1. **Controller-verified same night:** all repo-citable claims matched exactly (A1/A15 formulas, `products_table.sql` schema incl. `cj_pid`, FAQ's "within 1 hour" text, "Free Shipping Over $50" header copy). One [EXT] claim upgraded via web search: Shopify Payments is confirmed **not available in Israel**, making the higher-fee (~5.5%+$0.49) scenario very likely Guy's real case, not just a hedge — strengthens the $24-floor case further. **One inaccuracy caught:** Part 2 treats "the site promises 48h dispatch" as a live commitment in three places, but the actual live FAQ only promises "7-14 business days" shipping with no dispatch-time SLA at all — the 48h-dispatch language is Stage 16's *proposed* future copy (§4.3, gated on unapproved item I), not live text. Flagged in the tracker rather than silently corrected in Fable's doc.
